...
namespace StockIndicators.Tests
{
    ...
    public class ChandeleirTests : TestBase
    {
        ...
        public void GetChandleierTest()
        {
            ...
            // sample values (long)
            ChandelierResult a = longResult.Where(x => x.Date == DateTime.ParseExact("12/31/2018", "MM/dd/yyyy", null)).FirstOrDefault();
            ...
            ChandelierResult b = longResult.Where(x => x.Date == DateTime.ParseExact("12/6/2018", "MM/d/yyyy", null)).FirstOrDefault();
            Assert.AreEqual(false, b.IsExitCross);
            Assert.AreEqual(false, b.IsCrossed);

            ChandelierResult c = longResult.Where(x => x.Date == DateTime.ParseExact("12/7/2018", "MM/d/yyyy", null)).FirstOrDefault();
            Assert.AreEqual(true, c.IsExitCross);
            Assert.AreEqual(true, c.IsCrossed);

            ChandelierResult d = longResult.Where(x => x.Date == DateTime.ParseExact("12/17/2018", "MM/dd/yyyy", null)).FirstOrDefault();
            ...
            ChandelierResult e = shortResult.Where(x => x.Date == DateTime.ParseExact("12/31/2018", "MM/dd/yyyy", null)).FirstOrDefault();
